...
const CategoryNavigation = () => {
  ...
  return (
    <div className="w-full py-4 space-y-4 max-w-screen-xl mx-auto">
      {/* Top Navigation with Icons */}
      <div className="flex items-center justify-between w-full ">
        <div className="flex gap-2 overflow-x-auto scrollbar-hide pb-2 flex-grow">
          {tabs.map((tab) => (
            <button
              key={tab.id}
        ...
        {/* Twitter Handle - Fixed position on the right */}
        <div className="hidden md:flex items-center gap-1 px-3 py-1.5 bg-gray-100 rounded-full text-sm text-gray-700 ml-2 whitespace-nowrap">
      ...
      <div className="flex gap-2 pb-2 items-center justify-center">
        {categories.map((category) => (
          <button
            key={category}
            className="px-3 py-1.5 bg-white hover:bg-gray-50 text-gray-700 rounded-full text-sm whitespace-nowrap border border-gray-200 transition-colors"
          >
            {category}
          </button>
        ))}
