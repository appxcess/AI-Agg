    ...
    <header className="fixed top-0 left-0 w-full z-50 bg-white shadow-md">
      ...
      <div className="flex items-center justify-between px-4 md:px-6 py-4">
        <div className="flex items-center space-x-2">
          <img src={logo} alt="Toolify.ai Logo" className="h-8 md:h-10 w-auto" />
          <span className="text-lg md:text-xl font-bold text-gray-800">Toolify.ai</span>
        </div>
      ...
      {/* Mobile Menu */}
      {mobileMenuOpen && (
        <div className="md:hidden bg-white border-t border-gray-200 overflow-y-auto max-h-screen pb-6">
          ...
          <div className="px-4 py-2">
            ...
            <div className="border-b border-gray-100 py-2">
              ...
              >
                ...
                <FaChevronDown className={`transition-transform duration-300 ${activeDropdown === 'products' ? 'transform rotate-180' : ''}`} />
              </button>
                ...
                <div className="mt-2 pl-2 space-y-2">
                  {productItems.map((item, index) => (
                    <Link 
                      ...
                      onClick={toggleMobileMenu}
                    >
                      <div className="flex-shrink-0 w-6 h-6 bg-purple-50 rounded-full flex items-center justify-center">
                        {item.icon}
                      </div>
                      <div className="flex flex-col">
                        <span className="text-gray-900 font-medium">{item.title}</span>
            ...
            <Link 
              to="/category" 
              className="block text-gray-800 font-medium py-3 border-b border-gray-100"
              onClick={toggleMobileMenu}
            >
              {t("Category")}
            </Link>

            {/* Ranking Dropdown Mobile */}
            <div className="border-b border-gray-100 py-2">
              <button 
                className="flex items-center justify-between w-full text-gray-800 font-medium py-2"
                onClick={() => toggleDropdown('ranking')}
              >
                <span>{t("Ranking")}</span>
                <FaChevronDown className={`transition-transform duration-300 ${activeDropdown === 'ranking' ? 'transform rotate-180' : ''}`} />
              </button>
            ...
            {/* Social Listening Link Mobile */}
            <Link 
              to="/SocialListening" 
              className="flex items-center text-gray-800 font-medium py-3 border-b border-gray-100"
              onClick={toggleMobileMenu}
            >
              Social Listening
            ...
            {/* Submit & Advertise Dropdown Mobile */}
            <div className="border-b border-gray-100 py-2">
              <button 
                className="flex items-center justify-between w-full text-gray-800 font-medium py-2"
                onClick={() => toggleDropdown('submit')}
              >
                <span>{t("submitAdvertise")}</span>
                <FaChevronDown className={`transition-transform duration-300 ${activeDropdown === 'submit' ? 'transform rotate-180' : ''}`} />
              </button>
            ...
            <div className="pt-3 border-t border-gray-100">
              <div className="text-gray-800 font-medium mb-2">Language</div>
              <div className="grid grid-cols-2 gap-2">
                <button 
                  className={`px-3 py-2 rounded text-center ${selectedLanguage === 'EN' ? 'bg-blue-500 text-white' : 'bg-gray-100'}`}
                  onClick={() => changeLanguage("en", "EN")}
                >
                  English
                </button>
                <button 
                  className={`px-3 py-2 rounded text-center ${selectedLanguage === 'ES' ? 'bg-blue-500 text-white' : 'bg-gray-100'}`}
                  onClick={() => changeLanguage("es", "ES")}
                >
                  Español
                </button>
                <button 
                  className={`px-3 py-2 rounded text-center ${selectedLanguage === 'FR' ? 'bg-blue-500 text-white' : 'bg-gray-100'}`}
                  onClick={() => changeLanguage("fr", "FR")}
                >
                  Français
                </button>
                <button 
                  className={`px-3 py-2 rounded text-center ${selectedLanguage === 'DE' ? 'bg-blue-500 text-white' : 'bg-gray-100'}`}
                  onClick={() => changeLanguage("de", "DE")}
                >
                  Deutsch
                </button>
